...
def _divide_playcount_list (df, user_playcount_list, max_len):
    ...
    if user_playcount_list.shape[1] == 0:
        print ("This user didn't listen to any of the songs...")
        return [], []
    ...
    for i in range(len(sorted_list)):  
        if (sorted_list[i] == 1): # first song with a playcount of 1
            ...
            break
    if one_idx == -1 : 
        print ("No playcount of ones..pass")
        return [], []
...
if __name__ == '__main__':
    ...
    # For testing, select subset of data (20000 users, 10000 songs)
    test_users = 20000
    test_songs = np.random.choice(user_song_matrix.shape[1], 10000, replace=False)
    split_data(training_data_path, df, user_song_matrix[:20000, test_songs], 5)
    
    # get list of test songs for converting to mel
    np.save(training_data_path+ 'songs.npy', np.array(songs)[test_songs])
